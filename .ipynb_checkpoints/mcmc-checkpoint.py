...
# -----------------------------------------------------------------------------
# Node Class for computing probabilites
# -----------------------------------------------------------------------------

class ProbNode():
    """The Probability Node Class. """
    def __init__(self, parent = None, offspring = None, variables= None, parameters= None, prob_ops = 0.5):
        self.parent = parent
        self.offspring = offspring if offspring is not None else []
        #Creation of a dictionary of possible node values and its probabilites
        #We will initially assign the same probability to all operations, par_values, or variables
        self.ops_list = list([key for key in OPS])
        self.var_list = variables if variables is not None else ['x']
        self.par_list = parameters if parameters is not None else ['a']
        #self.par_var_prob_list = {ele: 1 / len(self.var_list + self.par_list) for ele in self.var_list + self.par_list}
        #self.ops_prob_list = {ele: 1 / len(self.ops_list) for ele in self.ops_list}
        self.ops_var_prob_list = {ele: 1 / len(self.var_list + self.ops_list) for ele in self.var_list + self.ops_list}
        self.par_prob_list = {ele: 1 / len(self.par_list) for ele in self.par_list}
        self.prob_ops = prob_ops
        self.prob_list = {}
        self.generate_prob_list(probability = prob_ops)
        return

    def add_son(self, new):
        self.offspring.append(new)
        new.parent = self

    def generate_prob_list(self, probability):
        if probability is not None:
            self.prob_ops = probability
        #probability is the probability associated to the group par_var_prob_list. Default probability = 0.5
        #dict1 = {k: v*(1 - probability) for k,v in self.par_var_prob_list.items()}
        #dict2 = {k: v*probability for k,v in self.ops_prob_list.items()}
        dict1 = {k: v*(1 - probability) for k,v in self.par_prob_list.items()}
        dict2 = {k: v*probability for k,v in self.ops_var_prob_list.items()}
        self.prob_list = dict1 | dict2

    def rand_mod(self):
        selected = choice(list(self.prob_list.keys()))
        #print(f"The chosen is {selected}\n")
        if selected in self.par_list:
            #gaussian step of 0.014 / number of parameters
            sigma = 0.014 / len(self.par_list)
        else:
            #guassian step of 0.01
            sigma = 0.01
        self.prob_list[selected] = max(10**(-6),self.prob_list[selected] + np.random.normal(0, sigma)) # to avoid negative values
        #normalization
        total_sum = sum(self.prob_list.values())
        normalized = {k: v / total_sum for k,v in self.prob_list.items()}
        self.prob_list = normalized
        
    '''
    def modify_prob_ops(self, dict):
        #modifies the probability of the given operations inside the ops_prob_list field
        self.modify_prob(self.ops_prob_list, dict)

    def modify_prob_var_par(self,dict):
        #modifies the probability of the given parameters/values inside the ops_prob_list field
        self.modify_prob(self.par_var_prob_list, dict)
    
    def modify_prob(self,prob_dict, new_values):
        old_prob = 1
        prob_dict.update(new_values)
        new_prob = sum(prob_dict.values())
        difference = new_prob - old_prob
        tot_prob = sum(value for key, value in prob_dict.items() if key not in new_values)
        for key in prob_dict:
            if key not in new_values:
                value = prob_dict[key]
                prob_dict[key] = value - (value*difference / tot_prob)
        self.generate_prob_list(probability = self.prob_ops)
    '''
    #copies the structure of a node and all its offspring
    def node_copy(self):
        node_copy = ProbNode(
            parent = None,
            offspring = [],
            variables= self.var_list,
            parameters= self.par_list,
            prob_ops = self.prob_ops,
        )
        node_copy.prob_list = self.prob_list.copy()
        node_copy.par_prob_list = self.par_prob_list.copy()
        node_copy.ops_var_prob_list = self.ops_var_prob_list.copy()
        
        for child in self.offspring:
            copy_child = child.node_copy()
            node_copy.add_son(copy_child)

        return node_copy

    #return the ALL the offspring of a node and the node itslef in a list format
    def node_run(self):
        node = self
        nodes = []
        def traverse(node):
    ...
    #Pick a random operation using the the prob_list probabilites
    def pick_op(self):
        keys = list(self.prob_list.keys())
        values = list(self.prob_list.values())
        return choices(keys, weights = values, k = 1)[0]
        
    #generate a random expression using the prob_list from all prob_node
    def pr(self, show_pow=False):
        value = self.pick_op()
        offspring = OPS.get(value,0)
        
        #if we have a selected operation but no more offspring, run again function to pick an operation without offpsring
        #same action when offspring is 2 but we have only 1 child node
        if (len(self.offspring) == 0 and offspring > 0) or (len(self.offspring) == 1 and offspring == 2):
            return self.pr(show_pow = show_pow)   
        elif offspring == 0:
            return '%s' % value
        elif offspring == 2:
            return '(%s %s %s)' % (self.offspring[0].pr(show_pow=show_pow),value, self.offspring[1].pr(show_pow=show_pow))
        else:
            if show_pow:
                return '%s(%s)' % (value, self.offspring[0].pr(show_pow = show_pow))
            else:
                if value == 'pow2':
                    ...
                    return '%s(%s)' % (value, self.offspring[0].pr(show_pow = show_pow))
'''           
    #generate the most plaussible expression: Rule: pick the operation with highes probability in each node and apply to offspring if necessary
    def max_expression(self, show_pow = False):
        value = max(self.prob_list, key = self.prob_list.get)
        offspring = OPS.get(value,0)
        
        #if we have a selected operation but no more offspring, run again function to pick an operation without offpsring
        #same action when offspring is 2 but we have only 1 child node
        if (len(self.offspring) == 0 and offspring == 1):
            newNode = ProbNode(parent = self)
            self.add_son(newNode)
            return '%s(%s)' % (value, newNode.max_expression(show_pow = show_pow))
        elif (len(self.offspring) == 0 and offspring == 2):
            newNode1 = ProbNode(parent = self)
            newNode2 = ProbNode(parent = self)
            self.add_son(newNode1)
            self.add_son(newNode2)
            return '(%s %s %s)' % (self.offspring[0].max_expression(show_pow=show_pow),value, self.offspring[1].max_expression(show_pow=show_pow))
        elif (len(self.offspring) == 1 and offspring == 2):
            newNode = ProbNode(parent = self)
            self.add_son(newNode)
            return '(%s %s %s)' % (self.offspring[0].max_expression(show_pow=show_pow),value, self.offspring[1].max_expression(show_pow=show_pow))
        elif offspring == 0:
            ...
            if show_pow:
                return '%s(%s)' % (value, self.offspring[0].max_expression(show_pow = show_pow))
            else:
                if value == 'pow2':
                    return '(%s ** 2)' % (self.offspring[0].max_expression(show_pow=show_pow))
                elif value == 'pow3':
                    return '(%s ** 3)' % (self.offspring[0].max_expression(show_pow=show_pow))
                else:
                    return '%s(%s)' % (value, self.offspring[0].max_expression(show_pow = show_pow))

'''
# -----------------------------------------------------------------------------
# Tree Class for computing probabilites
# -----------------------------------------------------------------------------

class ProbTree():
    """The Probability Tree Class. """
    """
        Generate a 100 node binary tree using the Nodes with the Probability Dictionaries
        Since we have a binary tree, the number of nodes will be of the form 2^n - 1 > max_size
        where n is the first integer such that the inequality holds
    """
    def __init__(self, tree, max_size = 100, variables = None, parameters = None, root = None):
        self.tree = tree
        self.parameters = parameters if parameters is not None else self.tree.parameters
        self.variables = variables if variables is not None else tree.variables
        myProbNode = ProbNode(offspring = [], parameters = self.parameters, variables = self.variables)
        self.max_size = max_size #max numbers of nodes
        self.root = root if root is not None else self.tree_builder()
        
        return

    #Builds by default a binary tree structure whose number of nodes is max_size
    def tree_builder(self):
        max_size = self.max_size
        n = 0
        while 2**n - 1 < max_size:
            n += 1
        numberNodes = 2**n - 1
        nodeList = [ProbNode(offspring = [], parameters = self.parameters, variables = self.variables) for i in range(numberNodes)]
        j = 0
        
        while j < ((numberNodes / 2) - 1):
            nodeList[j].add_son(nodeList[(j+1)*2 - 1])
            nodeList[j].add_son(nodeList[(j+1)*2 + 1 - 1])
            j += 1
            
        #return the root
        return nodeList[0]

    #Run through all nodes, check their current value and probability of its current value, return a list of all this probabilites
    def pr_tree_walk(self, probNode = None, Node = None, list_of_probs = None):
        #Stores in a string all the probabilites from each node
        Node = Node if Node is not None else self.tree.root
        probNode = probNode if probNode is not None else self.root
        list_of_probs = list_of_probs if list_of_probs is not None else []     
        list_of_probs.append(probNode.prob_list[Node.value])
        order = len(Node.offspring)
        
        for i in range(order):
            child_Node = Node.offspring[i]
            #In case the probNode had no sons, we'll create two additional ones
            if len(probNode.offspring) == 0:
                son1 = ProbNode(variables= self.variables, parameters= self.parameters, prob_ops = 0.5)
                son2 = ProbNode(variables= self.variables, parameters= self.parameters, prob_ops = 0.5)
                probNode.add_son(son1)
                probNode.add_son(son2)
                #With this we can update dynamically the probability tree from a base structure
            child_probNode = probNode.offspring[i]
            self.pr_tree_walk(probNode = child_probNode, Node = child_Node, list_of_probs = list_of_probs)
        return list_of_probs

    #Probability of a given expression using the tree
    def pr_tree(self):
        prob_list = self.pr_tree_walk()
        probability = 1
    ...
    def prob_expression(self, expression = None):
        if expression is not None:
            self.tree.build_from_string(expression)
        return self.pr_tree()
            
            

    #Replace the used tree for another
    def tree_change(self, newtree):
        if newtree is not None:
            self.tree = newtree

    #Function to create the copy of a tree
    def tree_copy(self):
        copy_tree = ProbTree(
            tree = self.tree,
            max_size = self.max_size,
            ...
            root = self.root.node_copy(),
        )
        return copy_tree

    def modify_random_prob(self):
        node_list = self.root.node_run() #obtain a list of the nodes
        rand_node = choice(node_list)  #pick a random node
        #print(f"the selected node is {rand_node}")
        rand_node.rand_mod()
        '''
        rand_ops = choice(list(OPS.keys())) #pick one random operation and modify its probability in the prob. dict of ops.
        print(f"Trying to change node {rand_node}, operation {rand_ops}\n")
        new_dict = {rand_ops : random()}
        rand_node.modify_prob_ops(new_dict) #update prob ops dictionary with new probability
        rand_node.generate_prob_list(random()) #randomly modify the probability of choosing an operation in general
        '''
        
    #generate a random expression based on the current probability tree
    def rand_func(self, show_pow = False):
        return self.root.pr(show_pow = show_pow)

    def max_pl_expr(self, show_pow = False):
        return self.root.max_expression(show_pow = show_pow)
